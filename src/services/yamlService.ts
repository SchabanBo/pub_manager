...
import { runCommand, runPubGetCommand } from "../helpers/utils";
...
export class YamlService {
  ...
  private initialize(): boolean {
    ...
    if (!workspaceFolder) {
      vscode.window.showErrorMessage(
        "Please open a pubspec.yaml file in a workspace before performing the update."
      );
      return false;
    }
  ...
  public getPubspecDependencies(): Package[] {
    if (Container.cache.packages.length > 0) {
      console.log("Using cached packages");
      return Container.cache.packages;
    }
    console.log("Fetching packages from pubspec.yaml");
    const lines = this.readLines();
    ...
    function parse(
      ...
      const line = lines.find(
        (line) => line.includes(dependency) && line.includes(currentVersion)
      );
      if (!line) {
        return null;
      }
    ...
    const dependencies = Object.keys(pubspec.dependencies).map((d) =>
      parse(d, pubspec.dependencies[d], false)
    );
    const devDependencies = Object.keys(pubspec.dev_dependencies).map((d) =>
      parse(d, pubspec.dev_dependencies[d], true)
    );
  ...
  public async getGitHistory(lineNumber: number): Promise<string> {
    try {
      ...
      let result = await runCommand(
        `git blame -L ${lineNumber},${lineNumber} pubspec.yaml`,
        { cwd }
      );
  ...
  public async updateAllPackages(): Promise<void> {
    ...
    for (const dependency of dependencies) {
      const toVersion =
        dependency.data?.resolvable ?? dependency.data?.latestVersion;
      if (!toVersion) {
        continue;
      }
      const canBeUpdated = semver.gt(toVersion, dependency.currentVersion);
      if (!canBeUpdated) {
        continue;
      }
      this.modifyPubspecContent(dependency.name, toVersion);
      dependency.currentVersion = toVersion;
    }
