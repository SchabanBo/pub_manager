...
export class Cache {
  ...
  public clear(): void {
    this.packages = [];
    this.ready = false;
    this.flutterVersion = undefined;
  }
}

export class Container {
  public static cache = new Cache();
  ...
  public static getExtensionContext(): vscode.ExtensionContext {
    return Container.instance.get<vscode.ExtensionContext>(
  ...
  public static getPanelMessagesService(): PanelMessagesService {
    return Container.instance.get<PanelMessagesService>(
      Container.panelMessagesService
    );
  }
